...
if __name__ == "__main__":
    ...
    seedPerturbRadius = volumeExtent/seedNumPerAxis/2 * 1 # meters = 600 um
    ...
    for vv in var:
        ...
        for rr in rands:
            ...
            # FEA
            thicknesses0, radii0 = vv * thicknessScaling, 100e-6 # Range of Thickness IS the determining factor of monotonic relationship
    ...
    ax1.set_title('Elastic Modulus vs Thickness')
    ax1.boxplot(E_bone.T)
    plt.xlabel("Plate Thickness (um)")
    plt.ylabel("Elastic Modulus (linear, compression)") 
    plt.xticks((np.arange(len(var))+1), (var*1e6).astype(int))
    ...
    plotter = pv.Plotter(shape=(1, 2), border=False, window_size=(2400, 1500))
    ...
    mesh = pv.PolyData(vertices, padPolyData(v.faces))
    ...
    plotter.camera_position = [(-41456.68805730983, -65909.3734707611, 23905.418788048253),
     (0.0, 0.0, 0.0),
     (0.16876817270434963, 0.24053571467115545, 0.9558555716475534)]
