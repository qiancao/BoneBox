...
if __name__ == "__main__":
    ...
    save_dir = "/data/BoneBox-out/topopt/lazy_v3_sweep_figures/"
    ...
    out_dir = "/data/BoneBox-out/topopt/lazy_v3_sweep_figures/randstate_" + str(randState) + saveNameAppend + "/"
    ...
    np.save(out_dir+"points", points)
    np.save(out_dir+"ppoints", ppoints)
    np.save(out_dir+"vertices", vor.vertices)
    np.save(out_dir+"uniqueFaces", uniqueFaces)
    ...
    for fea_iter in np.arange(Niters):
        ...
        indA = feaResult['nodeIndA']
        n0 = feaResult['nodes']
        d = feaResult['displacement']
        n1 = n0+d
        
        plt.figure()
        plt.plot(n0[:,0],n0[:,2],'b.')
        plt.plot(n1[:,0],n1[:,2],'r.')
        plt.axis("off")
        plt.close('all')
        
        displacementFaceA = np.mean(d[indA,2])
        
        print("Face A Displacement: " + str(displacementFaceA)) # TODO: This should always be negative
        ...
        pl.add_mesh(grid,show_edges=False, scalars=arrayVM, cmap=cmap, clim=(0.1,0.7))
        pl.show(window_size=(2000,2000),cpos=cpos,screenshot=out_dir+"volume_"+str(fea_iter)+".png")
        ...
        pl.show(window_size=(2000,2000),cpos=cpos, screenshot=out_dir+"volume_nocolor_"+str(fea_iter)+".png")
    ...
    #%% Visualize Iteration 0 and 1 histogram
    
    arrayVM0 = np.load(out_dir+"arrayVM_0.npy")
    arrayVM7 = np.load(out_dir+"arrayVM_7.npy")
    ...
    fig, ax1 = plt.subplots()
    ax1.hist(arrayVM0,bins=fig_vm, alpha=0.5, color = "r")
    ax1.hist(arrayVM7,bins=fig_vm, alpha=0.5, color = "b")
    ax1.set_xlabel('Von Mises Stress (MPa)')
    ax1.set_ylabel('# Elements (Voxels)')
    ax1.set_xlim(0,1.2)
    
    ax2 = ax1.twinx()
    plt.plot(fig_vm,fig_p,'k--',linewidth=2)
    ax2.set_ylabel('Bone Formation/Resorption Probability')
    ax2.set_xlim(0,1.2)
    
    plt.savefig(out_dir+"hist_"+str(fea_iter)+"_all.png",bbox_inches='tight')
    plt.close("all")
    
    #%% Visualize Voronoi points
    
    points = np.load(out_dir+"points.npy")
    ppoints = np.load(out_dir+"ppoints.npy")
    point_cloud = pv.PolyData(points)
    ppoint_cloud = pv.PolyData(ppoints)
    
    
    # point_cloud.plot(eye_dome_lighting=False)
    # ppoint_cloud.plot(eye_dome_lighting=True)
    
    plotter = pv.Plotter()
    plotter.add_mesh(point_cloud, color='black', point_size=15.,
                 render_points_as_spheres=True)
    plotter.add_mesh(ppoint_cloud, color='red', point_size=15.,
                 render_points_as_spheres=True)
    plotter.show_bounds(grid=True, location='back')
    plotter.show(window_size=(2000,2000), screenshot = out_dir+"seed points.png")
    
    #%% Visualize Voronoi faces
    
    # pvverts = vor.vertices.copy()
    # pvfaces = np.hstack(uniqueFaces)
    # pvsurf = pv.PolyData(pvverts, pvfaces)
    # # pvsurf.plot(scalars=np.arange(3), cpos=[-1, 1, 0.5])
    
    # plotter = pv.Plotter()
    # plotter.add_mesh(pvsurf)
    # plotter.show(window_size=(2000,2000), screenshot = out_dir+"uniqueFaces.png")
    
    #%% Visualize volume shape
    
    def cropHexMesh(xyz, cropSize = 20):
        # crops a hexahedral mesh
        
        volumeSize = 100
        
        inds = np.logical_and.reduce(np.logical_and((xyz-cropSize>0),((xyz+cropSize<volumeSize))),
                                          axis=1)
        
        return inds, xyz[inds,:]
    
    arrayVM0 = np.load(out_dir+"arrayVM_0.npy")
    xyz0 = np.load(out_dir+"xyz0.npy")
    arrayVM7 = np.load(out_dir+"arrayVM_7.npy")
    xyz7 = np.load(out_dir+"xyz7.npy")
    
    inds0, xyz0c = cropHexMesh(xyz0)
    inds7, xyz7c = cropHexMesh(xyz7)
    
    volume0 = np.zeros((100,100,100));
    volume7 = np.zeros((100,100,100));
    
    volume0[tuple(xyz0c.T)] = arrayVM0[inds0]
    volume7[tuple(xyz7c.T)] = arrayVM7[inds7]
    
    # def zeroSides(volume,size=28):
    ...
    # volume0 = zeroSides(np.load(out_dir+"volume_0.npy"))
    nodes0, elements0 = Voxel2HexaMeshIndexCoord(volume0)
    
    # volume7 = zeroSides(np.load(out_dir+"volume_7.npy"))
    nodes7, elements7 = Voxel2HexaMeshIndexCoord(volume7)
    ...
    # Each cell begins with the number of points in the cell and then the points composing the cell
    nodes, elements, arrayVM = nodes0, elements0, arrayVM0[inds0]
    points = nodes
    cells = np.concatenate([(np.ones(elements.shape[0],dtype="int64")*8)[:,None], elements],axis=1).ravel()
    celltypes = np.repeat(np.array([vtk.VTK_HEXAHEDRON]), elements.shape[0])
    offset = np.arange(elements.shape[0])*9
    ...
    pl = pv.Plotter()
    pl.add_mesh(grid, show_edges=False, scalars=arrayVM, cmap=cmap, clim=(0.1,0.7))
    pl.show(window_size=(3000,3000), screenshot = out_dir+"volume iteration 0.png")
    
    #%%
    
    # Each cell begins with the number of points in the cell and then the points composing the cell
    nodes, elements, arrayVM = nodes8, elements8, arrayVM7[inds7]
    points = nodes
    cells = np.concatenate([(np.ones(elements.shape[0],dtype="int64")*8)[:,None], elements],axis=1).ravel()
    celltypes = np.repeat(np.array([vtk.VTK_HEXAHEDRON]), elements.shape[0])
    offset = np.arange(elements.shape[0])*9
    ...
    pl = pv.Plotter()
    pl.add_mesh(grid,show_edges=True, scalars=arrayVM, cmap=cmap, clim=(0.1,0.7))
    pl.show(window_size=(3000,3000), screenshot = out_dir+"volume iteration 8.png")
        
        
    #%% Loop
    # for ss in [0]: #scaffoldSeeds[84:]:
    #     for uu in [2]:
