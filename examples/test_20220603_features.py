...
def computeNPS(rois,voxSize):
    """
    Compute noise power spectra from ROIsWW
...
def NPS2noise(NPS,seed=None):
    """
    Generates noise image by filtering white noise with power spectra
    ...
    F = NPS * (np.cos(2*np.pi*v) + 1j*np.sin(2*np.pi*v)) # random phase shifts
    f = np.fft.ifftn(F, axes=tuple(range(NPS.ndim))) # ifftn: 
