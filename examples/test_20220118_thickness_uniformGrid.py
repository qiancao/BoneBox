...
if __name__ == "__main__":
    ...
    # var = np.linspace(1, 2, 3)
    var = np.linspace(1e-8, 2e-8, 5) # interesting, monotonic relationship only observed after 1e-8
    # var = np.linspace(0, 90, 3)
    rands = np.arange(1)
    displacements = []
    ...
    for vv in var:
        ...
        for rr in rands:
            ...
            v = generateVoronoiSkeleton(Sxyz=(100,100,100),Nxyz = (8,8,8), randState=rr, Rxyz = 3)
            vertices = v.vertices
            edges = np.array([])
            ...
            # Boundary conditions
            verticesForce = np.nonzero(vertices[:,2] > 30)[0]
            verticesFixed = np.nonzero(vertices[:,2] < -30)[0]
            forceVector = np.array([0,0,-1e5]).astype(np.double)
            
            # FEA
            thicknesses0, radii0 = vv, 100e-6
    ...
    ax1.set_title('Displacement vs Thickness')
    ax1.boxplot(displacements.T)
    plt.xlabel("Plate Thickness")
    plt.ylabel("Displacement of Moving Face") 
    plt.xticks(np.arange(len(var))+1, var)
