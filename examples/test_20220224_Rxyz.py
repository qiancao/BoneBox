...
if __name__ == "__main__":
    ...
    # Output directory
    outDir = "/data/BoneBox-out/test_20220224_Rxyz/"
    ...
    for vv in var:
        ...
        for rr in rands:
            ...
            # volume = voxelize_plates(verticesFE/verticesScaling, facesFE[:,:3], dims, spacing, thicknessVoxels*5)
            
            # sdf = volume_sdf(verticesFE/verticesScaling, facesFE[:,:3], shellThicknesses/thicknessScaling, 
            #                   edges, np.array([]), dims, spacing, 10*np.max(shellThicknesses/thicknessScaling))
            
            # Constant thickness
            sdf = volume_sdf(verticesFE/verticesScaling, facesFE[:,:3], shellThicknesses/thicknessScaling, 
                              edges, np.array([]), dims, spacing, dims[0]*spacing[0]/100)
            sdf[np.isnan(sdf)] = np.nanmax(sdf) # set unclassified boundaries to nanmax
            
            # sdf_platten = addPlatten(sdf, 10, plattenValue=np.min(sdf), airValue=np.max(sdf), trimVoxels=30)
            ...
            # # Variable thickness
            # sdf_var = volume_sdf(verticesFE/verticesScaling, facesFE[:,:3], shellThicknesses/thicknessScaling * np.max(thicknessVoxels)/5, 
            #                   edges, np.array([]), dims, spacing, dims[0]*spacing[0]/100)
            # sdf_var[np.isnan(sdf_var)] = np.nanmax(sdf_var) # set unclassified boundaries to nanmax
            
            # sdf_platten_var = addPlatten(sdf_var, 10, plattenValue=np.min(sdf_var), airValue=np.max(sdf_var), trimVoxels=30)
            # vertices_sdf, faces_sdf, normals, values = Voxel2SurfMesh(sdf_platten_var, voxelSize=spacing, origin=None, level=-0.8e-5, step_size=1, allow_degenerate=False)
            # tmesh = trimesh.Trimesh(vertices_sdf, faces=faces_sdf)
            # surf = pv.wrap(tmesh)
            # surf.plot()
            
            # volume = sdf_platten_var < 0
            
            
            # plotter = pv.Plotter(shape=(1, 1), border=False, window_size=(2400, 1500))
            # plotter.background_color = 'w'
            # plotter.enable_anti_aliasing()
            # plotter.add_mesh(surf)
            # plotter.show(screenshot=outDir+f'isosurface.png')
            ...
            #     dims=dims,
            #     spacing=spacing,
            #     origin=np.array(dims)/2,
            # )
            # grid.point_data["values"] = sdf.flatten(order="F") 
            # mesh = grid.contour(0, method='marching_cubes')
            # mesh.plot()

            # plot SDFgrid
            # sdf_inf = sdf
            # sdf_inf[np.isnan(sdf)] = 10
            # grid = pv.UniformGrid(dims=dims, spacing=spacing, origin=(0,0,0))
            # grid.point_data['values'] = sdf_inf.ravel(order='F')
            # out = grid.contour(method='marching_cubes')
