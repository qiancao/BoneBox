...
def renderFEA(imgName, strainsBar, strainsShell4Face, v, verticesForce, verticesFixed):
    ...
    plotter.camera_position = [(0.03156407591507407, -0.023281266066764967, 0.011491945321349735),
     (6.512945574623516e-05, -9.595209762820658e-06, 2.295668191710197e-05),
     (-0.23200960089564612, 0.15891008584428062, 0.9596453145351185)]
...
if __name__ == "__main__":
    ...
    for rr in randStates:
        ...
        forceVector = np.array([0,0,-1e-3]).astype(np.double)
        ...
        def shifted_sigmoid(x, a=0, beta=1.):
            x = x - a
            return 2 / (1 + np.exp(-(beta*x)))
        
        beta = 1e-8
        thicknessMultiplier = np.ones(shellThicknesses.shape)
        
        for it in range(iters):
            
            mults = np.zeros((iters,len(strainsShellQuad)))
        
            thicknessMultiplier = shifted_sigmoid(strainsShellQuad, np.nanmedian(strainsShellQuad), beta) * thicknessMultiplier
            mults[it,:] = thicknessMultiplier
            
            verticesFE1, strainsBar, strainsShell = computeFEARodPlateLinear(verticesFE, edges, facesFE,
                                         verticesForce, verticesFixed, forceVector,
                                         elasticModulus, poissonRatio, density,
                                         barAreas, shellThicknesses * thicknessMultiplier, shellFiberAngles,
                                         verticesScaling=verticesScaling,
                                         plateThicknessScaling=plateThicknessScaling)
            ...
            stiffnesses[it+1,rr] = np.linalg.norm(forceVector) / displacement
