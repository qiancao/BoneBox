"""
Phantoms.TrabeculaeVoronoi_Shi
...
def makeSeedPointsCartesian_LineorArc(dx, dy, dz, axy, axz, ayx, ayz, azx, azy, Sxyz,rad=1000000):
    # Unit increament of x y z axis
    xIncreament = [math.cos(axy/180*np.pi)*math.cos(axz/180*np.pi), \
                    math.sin(axy/180*np.pi)*math.cos(axz/180*np.pi), \
                    math.sin(axz/180*np.pi)]
    xIncreament = xIncreament/np.linalg.norm(xIncreament)
    
    yIncreament = [math.sin(ayx/180*np.pi)*math.cos(ayz/180*np.pi), \
                    math.cos(ayx/180*np.pi)*math.cos(ayz/180*np.pi), \
                    math.sin(ayz/180*np.pi)]
    yIncreament = yIncreament/np.linalg.norm(yIncreament)
    
    zIncreament = [math.sin(azx/180*np.pi)*math.cos(azy/180*np.pi), \
                    math.sin(azy/180*np.pi), \
                    math.cos(azx/180*np.pi)*math.cos(azy/180*np.pi)]
    zIncreament = zIncreament/np.linalg.norm(zIncreament)
    
    points = np.array([[0,0,0]])
    tempPoint = [0,0,0]
    tempPoint_N = [0,0,0]
    oriPoint = tempPoint
    oriPoint_N = tempPoint
    
    ct = 0
    # While points are not out of bound
    while (abs(tempPoint[0])<Sxyz[0]/2) & (abs(tempPoint[1])<Sxyz[1]/2) & (abs(tempPoint[2])<Sxyz[2]/2) & \
            (abs(tempPoint_N[0])<Sxyz[0]/2) & (abs(tempPoint_N[1])<Sxyz[1]/2) & (abs(tempPoint_N[2])<Sxyz[2]/2) :
        points = np.append(points,[tempPoint],axis = 0)
        points = np.append(points,[tempPoint_N],axis = 0)
        ct = ct+1
        if rad == 0:
            # Increament point along x dir one increament
            tempPoint = oriPoint + np.multiply(xIncreament,dx)*ct
            tempPoint_N = oriPoint_N - np.multiply(xIncreament,dx)*ct
        else:
            angle = dx/(2*np.pi*rad)*360
            centerPhere = [0,-rad,0]
            tempPoint[0] = math.sin(ct*angle/180*np.pi)*rad+centerPhere[0]
            tempPoint[1]= math.cos(ct*angle/180*np.pi)*rad+centerPhere[1]
            tempPoint_N[0] = -math.sin(ct*angle/180*np.pi)*rad+centerPhere[0]
            tempPoint_N[1]= math.cos(ct*angle/180*np.pi)*rad+centerPhere[1]
        
    xpoints = points
    for xct in range(xpoints.shape[0]):
        tempPoint = xpoints[xct]
        tempPoint_N = xpoints[xct]
        while (abs(tempPoint[0])<Sxyz[0]/2) & (abs(tempPoint[1])<Sxyz[1]/2) & (abs(tempPoint[2])<Sxyz[2]/2) :
            points = np.append(points,[tempPoint],axis = 0)
            tempPoint = tempPoint + np.multiply(yIncreament,dy)
        
        while (abs(tempPoint_N[0])<Sxyz[0]/2) & (abs(tempPoint_N[1])<Sxyz[1]/2) & (abs(tempPoint_N[2])<Sxyz[2]/2) :
            points = np.append(points,[tempPoint_N],axis = 0)
            tempPoint_N = tempPoint_N - np.multiply(yIncreament,dy)
    
    ypoints = points
    for yct in range(ypoints.shape[0]):
        tempPoint = ypoints[yct]
        tempPoint_N = ypoints[yct]
        while (abs(tempPoint[0])<Sxyz[0]/2) & (abs(tempPoint[1])<Sxyz[1]/2) & (abs(tempPoint[2])<Sxyz[2]/2) :
            points = np.append(points,[tempPoint],axis = 0)
            tempPoint = tempPoint + np.multiply(zIncreament,dz)
        
        while (abs(tempPoint_N[0])<Sxyz[0]/2) & (abs(tempPoint_N[1])<Sxyz[1]/2) & (abs(tempPoint_N[2])<Sxyz[2]/2) :
            points = np.append(points,[tempPoint_N],axis = 0)
            tempPoint_N = tempPoint_N - np.multiply(zIncreament,dz)
...
def findUniqueEdgesAndFaces(vor, ind, Sxyz, maxEdgePerFace=8):
    """
